import {TransformerNestedStack, TransformerPluginBase} from "@aws-amplify/graphql-transformer-core";
import {
...
export default class CountTransformer extends TransformerPluginBase implements TransformerPluginProvider {
    ...
    generateResolvers = (ctx: TransformerContextProvider) => {
        ...
        for (const model of this.models) {
            ...
            const tableDataSource = ctx.dataSources.get(model) as appsync.DynamoDbDataSource;
            const table = tableDataSource.ds.dynamoDbConfig as appsync.CfnDataSource.DynamoDBConfigProperty;

            // Allow the lambda to access this table
            funcRole.addToPolicy(new iam.PolicyStatement({
                actions: [ "dynamodb:Scan" ],
                effect: iam.Effect.ALLOW,
                resources: [ `arn:aws:dynamodb:${table.awsRegion}:${stack.account}:table/${table.tableName}` ]
            }));
