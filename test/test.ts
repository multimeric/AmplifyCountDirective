...
import {makeScanInput, primitivesToString, notEmptyObject, CountResolverEvent, DynamoFilter} from "../handler";
...
describe("makeScanInput", () => {
    test("empty properties", () => {
        expect(makeScanInput(makeAppSyncEvent({
                expression: '',
                expressionNames: {},
                expressionValues: {}
            }),
    ...
    test("non empty properties", () => {
        expect(makeScanInput(makeAppSyncEvent({
                expression: '((#int_field = :and_0_int_field_eq) AND (attribute_exists(#bool_field)))',
                expressionNames: {'#bool_field': 'bool_field', '#int_field': 'int_field'},
                expressionValues: {':and_0_int_field_eq': [{N: "123"}]}
            }),
            undefined
        )).toEqual(expect.objectContaining({
            FilterExpression: '((#int_field = :and_0_int_field_eq) AND (attribute_exists(#bool_field)))',
            ExpressionAttributeNames: {'#bool_field': 'bool_field', '#int_field': 'int_field'},
...
test("primitivesToString", () => {
    ...
    expect(primitivesToString({a: {b: {c: 3}}})).toEqual({a: {b: {c: "3"}}});
})
